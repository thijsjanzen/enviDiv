#' collect reference table based on previously generated reference tables for each model
#' @param path path of directory where each subdirectory [1, 2, 3] is located
#' @param return_full_table should the function only return summary stats, or also parameters?
...
read_reference_table <- function(path,
  ...
  if(!return_full_table) {
    ...
    ref_table <- dplyr::select(reference_table, c("nltt", "gamma", "mbr", "num_lin",
                                           "beta", "colless", "sackin", "ladder", "cherries", "ILnumber",
                                           "pitchforks", "stairs",
                                           "spectr_eigen", "spectr_asymmetry", "spectr_peakedness"))
