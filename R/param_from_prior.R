...
param_from_prior_exp <- function(empty_input) {
  output <- c()
  output[1] <- stats::rexp(n = 1, rate = 0.05)  # extinction
  output[2] <- stats::rexp(n = 1, rate = 0.1)  # symp spec high
  output[3] <- stats::rexp(n = 1, rate = 0.1)  # symp spec low
  output[4] <- stats::rexp(n = 1, rate = 0.1)  # allo spec
  output[5] <- stats::rexp(n = 1, rate = 0.05)  # jiggle
  output[6] <- sample(1:3, 1)
  output[7] <- 1
