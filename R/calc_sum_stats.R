...
calc_sum_stats <- function(focal_tree, emp_tree) {
  ...
  sackinStat <- function(tree)  {
    if(!ape::is.rooted(tree)) return(NA)
    # if(!is.binary.tree(tree)) return(NA)
    tree <- ape::multi2di(tree)
    x <- apTreeshape::sackin(apTreeshape::as.treeshape(tree),norm=NULL)
    return(x)
  }

  output[5] <- betaStat(focal_tree) # beta
  output[6] <- collessStat(focal_tree) # colless
  output[7] <- sackinStat(focal_tree) # sackin
